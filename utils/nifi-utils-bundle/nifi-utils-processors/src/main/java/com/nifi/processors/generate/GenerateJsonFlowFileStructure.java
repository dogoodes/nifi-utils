...
public class GenerateJsonFlowFileStructure extends AbstractProcessor {
    ...
    public GenerateJsonFlowFileStructure(){}
    ...
    public void onTrigger(final ProcessContext context, final ProcessSession session) throws ProcessException {
        ...
        JsonElement jsonAttribute = null;
        ...
        if (flowFile.getSize() > 0) {
            ...
            jsonContent = gson.fromJson(contentString, JsonElement.class);
        } else
        ...
        if (attributes != null && !attributes.isEmpty()) {
            ...
            for(final Map.Entry<String, String> attribute : attributes.entrySet()){
                jsonObject.addProperty(attribute.getKey(), attribute.getValue());
            }
            jsonAttribute = gson.fromJson(jsonObject.toString(), JsonElement.class);
        }

        final FlowFileStructure flowFileStructure = new FlowFileStructure.Builder().attribute(jsonAttribute).content(jsonContent).build();

        flowFile = session.write(flowFile, new OutputStreamCallback() {
            @Override
            public void process(OutputStream out) throws IOException {
                out.write(gson.toJson(flowFileStructure).getBytes(StandardCharsets.UTF_8));
            }
        });
